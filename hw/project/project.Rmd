...
This analysis explores the differences in regular drug consumption across age groups and what their trends are. By exploring these, we aim to identify which age groups should heavily be focused on when considering the most common drug consumed. 

Ultimately, this analysis finds that the most consumed drugs were caffeine, chocolate (which can cause abnormal behaviors and other sensations that can be considered an addictive substance [“Chocolate: Food or Drug? - University of Arizona”]), and alcohol among individuals aged 18-24. The premises of this analysis will focus on alcohol consumption as it is intuitively less harmless than the two. Overall we found that alcohol consumption decreases significantly with increased age as the data is further analyzed.
...
Our data came from a Drug consumption dataset (Fehrman, Egan, and Mirkes 2015, https://doi.org/10.24432/C5TC7S). This dataset had information on many different demographics and their consumption of various drug types, some legal and some illegal. The raw data contains many columns and almost 2000 rows (individuals); however for this study we only used columns that corresponded to age group (Age) and alcohol consumption (Alcohol). 
